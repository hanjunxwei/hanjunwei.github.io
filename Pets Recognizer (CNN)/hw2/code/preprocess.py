...
def get_data(file_path, first_class, second_class):
    ...
    ## Extract class inputs
    first_class_data = np.array(inputs)[np.nonzero(np.array(labels)== first_class)]/255
    second_class_data = np.array(inputs)[np.nonzero(np.array(labels)== second_class)]/255
    
    ## Labels One-hot vector
    y = tf.one_hot(np.concatenate((0 * np.ones(first_class_data.shape[0]),1 * np.ones(second_class_data.shape[0]))),2)
    
    ## Reshape input
    x = np.concatenate((first_class_data,second_class_data))
    x = tf.reshape(x, (-1, 3, 32 ,32))
    x = tf.transpose(x, perm=[0,2,3,1])
    # shuffle inputs and labels
    ## get index 
    rand_index = tf.random.shuffle(np.array(range(y.shape[0])))
    ## shuffle
    x = tf.gather(x, rand_index)
    y = tf.gather(y, rand_index)
